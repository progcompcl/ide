...
class API {
  constructor(options) {
    ...
    this.hostWrite = options.hostWrite;
    this.clangFilename = options.clang || '/wasm/clang';
    this.lldFilename = options.lld || '/wasm/lld';
    this.sysrootFilename = options.sysroot || '/wasm/sysroot.tar';
    ...
    this.memfs = new MemFS({
      ...
      memfsFilename : options.memfs || '/wasm/memfs',
    });
  ...
  async run(module, ...args) {
    // Don't print the wasm filename to avoid "> test.wasm" output
